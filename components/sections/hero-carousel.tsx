    ...
    <div className="relative w-full overflow-hidden py-10 md:py-24 flex flex-col items-center">
      ...
      <div className="text-center z-10 mb-8 md:mb-12 flex flex-col items-center px-4">
        ...
        >
          ...
          <span className="text-[#D4FF3E]">MOVIEHUB</span>
        </motion.h1>
        ...
        >
          <Link
            href={`/watch/${activeMovie?.mediaType || "movie"}/${activeMovie?.id}`}
            className="bg-[#D4FF3E] text-black font-semibold px-8 py-3.5 rounded-full hover:bg-white hover:scale-105 transition-all text-sm md:text-lg"
          >
            Get started
          </Link>
          <Link
            href="/live-tv"
            className="border border-[#8A8A8E] text-white font-semibold px-8 py-3.5 rounded-full hover:border-[#D4FF3E] hover:text-[#D4FF3E] transition-all text-sm md:text-lg"
          >
            Live TV
          </Link>
        </motion.div>
        ...
        <AnimatePresence mode="popLayout" custom={direction}>
          ...
          >
            <div className="w-full h-full rounded-[3rem] md:rounded-[4rem] overflow-hidden relative shadow-2xl transition-all group-hover:opacity-100">
              <Image
                src={getTMDBImageUrl(
                  prevMovie.backdropPath || prevMovie.posterPath,
                  780,
                )}
                alt="Previous"
                fill
          ...
          >
            <div className="w-full h-full rounded-[3rem] md:rounded-[4rem] overflow-hidden relative shadow-2xl transition-all group-hover:opacity-100">
              <Image
                src={getTMDBImageUrl(
                  nextMovie.backdropPath || nextMovie.posterPath,
                  780,
                )}
                alt="Next"
                fill
          ...
          >
            ...
            >
              <div className="w-full h-full rounded-[3rem] md:rounded-[4rem] overflow-hidden relative shadow-[0_20px_60px_rgba(0,0,0,0.8)] border border-white/5 cursor-pointer group">
                <Image
                  src={getTMDBImageUrl(
                    activeMovie.backdropPath || activeMovie.posterPath,
                    1280,
                  )}
                ...
                <div className="absolute inset-0 p-6 md:p-12 flex flex-col justify-end">
                  <h2 className="text-3xl md:text-5xl lg:text-6xl font-black drop-shadow-xl text-white tracking-tight">
                    {activeMovie.title}
                  </h2>
                  <div className="flex flex-wrap items-center gap-3 mt-4 text-sm font-semibold">
                    <span className="flex items-center gap-1 bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-white">
                      <span className="text-[#D4FF3E]">★</span>{" "}
                      {Number(activeMovie.voteAverage).toFixed(1)}/10
                    </span>
                ...
                <div className="absolute z-30 flex items-center justify-between w-full px-4 md:px-8 top-1/2 -translate-y-1/2 pointer-events-none">
                  <button
                    ...
                    className="pointer-events-auto w-12 h-12 md:w-16 md:h-16 rounded-full border border-white/10 bg-black/50 backdrop-blur-md flex items-center justify-center text-white hover:bg-[#D4FF3E] hover:text-black hover:border-transparent transition-all duration-300 shadow-xl -ml-6 md:-ml-12 opacity-0 group-hover:opacity-100 hover:scale-105"
                  >
                  ...
                  <button
                    ...
                    className="pointer-events-auto w-12 h-12 md:w-16 md:h-16 rounded-full border border-white/10 bg-black/50 backdrop-blur-md flex items-center justify-center text-white hover:bg-[#D4FF3E] hover:text-black hover:border-transparent transition-all duration-300 shadow-xl -mr-6 md:-mr-12 opacity-0 group-hover:opacity-100 hover:scale-105"
                  >
