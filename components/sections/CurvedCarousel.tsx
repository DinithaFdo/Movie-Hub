...
export function CurvedCarousel({ movies }: CurvedCarouselProps) {
  ...
  const paginate = (direction: number) => {
    ...
    setCurrentIndex(
      (prev) => (prev + direction + movies.length) % movies.length,
    );
  };
  ...
  return (
    <div className="relative w-full overflow-hidden py-8 md:py-16 mt-24 md:mt-32 flex flex-col items-center justify-start min-h-[700px] md:min-h-[850px] z-10">
      ...
      */}
      <div className="relative w-full h-[350px] md:h-[550px] flex items-center justify-center perspective-[2000px] transform-style-3d">
        <AnimatePresence initial={false}>
          {offsets.map((offset) => {
            const index =
              (currentIndex + offset + movies.length) % movies.length;
        ...
        <div className="absolute right-[4%] md:right-[26%] z-50 flex items-center bg-white/10 backdrop-blur-2xl border border-white/20 rounded-full shadow-2xl p-1 pointer-events-auto">
          <motion.button
            onClick={() => paginate(-1)}
            whileHover={{
              backgroundColor: "rgba(212, 255, 62, 0.9)",
              color: "#000",
              scale: 1.05,
            }}
            whileTap={{ scale: 0.95 }}
